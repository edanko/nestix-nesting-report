...
namespace Report.FastClone.Internal
{
    ...
    {
        static readonly MethodInfo FieldInfoSetValueMethod =
            typeof(FieldInfo).GetMethod("SetValue", new[] { typeof(object), typeof(object) });

        static readonly MethodInfo DictionaryContainsKey = typeof(Dictionary<object, object>).GetMethod("ContainsKey");
        static readonly MethodInfo DictionaryGetItem = typeof(Dictionary<object, object>).GetMethod("get_Item");

        static readonly MethodInfo GetTypeClonerMethodInfo =
            typeof(Cloner).GetMethod("GetTypeCloner", BindingFlags.NonPublic | BindingFlags.Static);

        static readonly MethodInfo GetTypeMethodInfo = typeof(object).GetMethod("GetType");

        static readonly MethodInfo InvokeMethodInfo =
        ...
        internal static Expression CreateSetFieldExpression(Expression clone, Expression value, FieldInfo fieldInfo)
        {
            ...
            if (fieldInfo.IsInitOnly)
                return Expression.Call(Expression.Constant(fieldInfo), FieldInfoSetValueMethod, clone,
            ...
            FieldInfo fieldInfo, ParameterExpression objectDictionary)
        {
            ...
            return Expression.IfThenElse(
                Expression.Call(objectDictionary, DictionaryContainsKey, originalField),
                CreateSetFieldExpression(clone,
                    Expression.Convert(Expression.Call(objectDictionary, DictionaryGetItem, originalField),
                        fieldInfo.FieldType), fieldInfo),
                CreateSetFieldExpression(clone,
                    Expression.Convert(
                        Expression.Call(
                            Expression.Call(GetTypeClonerMethodInfo,
                                Expression.Call(originalField, GetTypeMethodInfo)), InvokeMethodInfo, originalField,
                            objectDictionary), fieldInfo.FieldType), fieldInfo)
            );
        }
        ...
        internal static Expression CreateCopyComplexArrayTypeFieldExpression(Expression sourceField,
            Expression targetField, Type type, ParameterExpression objectDictionary)
        {
            return Expression.IfThenElse(
                Expression.Call(objectDictionary, DictionaryContainsKey, sourceField),
                Expression.Assign(targetField,
                    Expression.Convert(Expression.Call(objectDictionary, DictionaryGetItem, sourceField), type)),
                Expression.Assign(targetField,
                    Expression.Convert(
                        Expression.Call(
                            Expression.Call(GetTypeClonerMethodInfo, Expression.Call(sourceField, GetTypeMethodInfo)),
                            InvokeMethodInfo, sourceField, objectDictionary), type))
            );
        }
    }
}